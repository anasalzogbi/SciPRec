	...
	for (min_sim, max_sim, peers) in parameters:
		...
		for fold in range(k_folds):
			...
			for pair in pairs:
				...
				i += 1
			#print("**** User {}, Training papers: {}, test paper: {}, pairs: {} ".format(user, len(user_training_ratings), len(user_fold_test_indices), len(pairs) ) )
			#print("*** PAIRS BUILT ***")
			feature_vectors = np.array(feature_vectors)
			# Using grid search for fitting hyper barameters (Penalty, C)
			tuned_parameters = [{'penalty': ['l1', 'l2'], 'C': [0.01, 1, 10]}]
			#print("***** Grid search")
			grid_clf = GridSearchCV(LinearSVC(dual=False, tol=0.0001, random_state=41), tuned_parameters, cv=2, scoring='recall')
			...
			#test_documents, test_indices = get_test_documents(user_fold_test_indices, user, document_matrix_shared)
			...
			for recall_x in recall_xs:
				fold_results.append(calculate_top_recall(predictions, user_fold_test_indices, recall_x, user_test_ratings))
...
if sim_min_end <= sim_min_st :
	print "sim_min_end {} must be greater than sim_min_st {}".format(sim_min_end, sim_min_st)
	sys.exit()
...
print ("*** Documents: {}".format(document_matrix_shared.shape))
